...
fn main() -> anyhow::Result<()> {
    ...
    println!("{}" , call(&mut tools, "browser", json!({
    ...
    let out = call(&mut tools, "browser", json!({
        "intent":"read", "command":"read"
    }))?;
    println!("[read] {} chars, has FOREWORD: {}", out.len(), out.contains("Foreword"));
    let shot = call(&mut tools, "browser", json!({

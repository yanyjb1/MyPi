//! 语义动作 —— 最终消费者（输入子区）对物理事件的解释。
//!
//! `keys.rs`（全局）只上报"按了什么"；"这意味着什么"归**最终消费者**。
//! 编辑键的最终消费者是输入子区的编辑器/补全管线，所以 Action 枚举、
//! KeyContext（翻译所需的子区状态）、translate（KeyEvent → Action 的
//! 语义映射）都住在这里。Zone 只转发物理事件，不解释。
...
pub struct KeyContext {
    ...
    // Waiting for the model reply. Set by the app from the wire's `state`
    // (busy) / `stream` (active) frames — see `KeyContext::for_input`.
    pub streaming: bool,
...
impl KeyContext {
    /// The app-side view: everything except `editor_empty` comes from the
    /// wire state (daemon-owned), editor_empty from the local editor.
    pub fn for_input(editor_empty: bool, streaming: bool, popup_open: bool) -> Self {
        Self {
            editor_empty,
            streaming,
            // 补全弹窗开着 = ↑↓/Tab/Enter/Esc 的解释权已经不在编辑器手里。
            // 这个位子以前恒为 false（没人填），于是"弹窗开着按回车"被当成
            // 提交——半截 `/switch ` 就这么发出去了。
            popup_open,
            ..Default::default()
        }
    }

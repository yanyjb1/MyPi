...
use crate::server::wire::CommandInfo;
...
pub fn candidate(text: &str, cursor: usize) -> Option<(usize, String)> {
    ...
    // 回扫取触发词：从光标往回扫到最近的硬边界。触发前置字符**只能是
    // 空格**（用户决策）——换行/tab 后面的词不是补全触发词。
    ...
    // 触发前置守卫：光标前第一个字符（触发词的第一个字符之前）必须是空格
    // 或词起始于文本开头（行首斜杠命令）。
...
pub fn complete(
    ...
    if at_line_start && input.starts_with('/') {
        return complete_commands(input, table);
    }
...
// 斜杠命令补全：`/mod` → `/model`。
//
// 表**不是这里的**：它由服务端在 `hello_ok` 里下发（`CommandInfo`），前端只
// 消费元数据。以前这里有一份自己的 `COMMANDS` 常量——两份表就是漂移的开始。
//
// 参数候选由调用方注入（模型 id 来自运行时配置，路径复用文件补全），这里只
// 负责命令名与前缀过滤。
pub fn complete_commands(input: &str, table: &[CommandInfo]) -> Vec<Completion> {
    let mut out: Vec<Completion> = table
        .iter()
...
/// 按名字或别名查一条命令（表来自服务端）。
pub fn find<'a>(table: &'a [CommandInfo], name: &str) -> Option<&'a CommandInfo> {
    table
        .iter()
        .find(|c| c.name == name || c.aliases.iter().any(|a| a == name))
}

/// 参数的形状：服务端下发的是字符串（老前端也能渲染没见过的命令），这里转回枚举。
...
impl ArgKind {
    pub fn of(cmd: &CommandInfo) -> ArgKind {
        match cmd.args.as_str() {
            "model_id" => ArgKind::ModelId,
            "profile_name" => ArgKind::ProfileName,
            "path" => ArgKind::Path,
            "text" => ArgKind::Text,
            _ => ArgKind::None,
        }
...
pub fn complete_files(input: &str, cwd: &Path, home: &Path) -> Vec<Completion> {
    ...
    // name_part 的点容忍形式：打 "env" 也命中 ".env"（无隐藏文件概念，
    // 显式决策）；同时取小写供不区分大小写匹配。
    let lower_part = name_part.to_lowercase();
    let dotless_part = name_part.strip_prefix('.').unwrap_or(name_part).to_lowercase();

    // 混合过滤（用户决策）：精确前缀 + 模糊子序列一起收，前缀命中
    // 排前。`src/` 触发后打成 "mian" 仍能捞回 main.rs。
    fn is_subsequence(needle: &str, hay: &str) -> bool {
        let mut it = hay.chars();
        needle.chars().all(|n| it.any(|h| h == n))
    }

    /// 打错字兜底：needle 的字符全部出现在 hay 里（不要求顺序，
    /// 覆盖相邻换位 mian↔main 这类子序列抓不住的错）。
    fn is_bag_match(needle: &str, hay: &str) -> bool {
        let mut hay_chars: Vec<char> = hay.chars().collect();
        needle.chars().all(|n| {
            if let Some(pos) = hay_chars.iter().position(|&h| h == n) {
                hay_chars.remove(pos);
                true
            } else {
                false
            }
    ...
    let mut out: Vec<(Rank, Completion)> = Vec::new();
    for entry in entries.flatten() {
        let name = entry.file_name().to_string_lossy().to_string();
        let lower_name = name.to_lowercase();
        // 前缀档：直接前缀 + 点容忍（含大小写不敏感）。
        let prefix_hit = name.starts_with(name_part)
            || (name.starts_with('.') && name[1..].starts_with(name_part))
            || lower_name.starts_with(&lower_part)
            || (lower_name.starts_with('.') && lower_name[1..].starts_with(&dotless_part));
        // 模糊档：子序列（按序）或全字符兜底（覆盖 mian↔main 换位）。
        let fuzzy_hit = !lower_part.is_empty()
            && (is_subsequence(&lower_part, &lower_name)
                || is_bag_match(&lower_part, &lower_name));
        if !prefix_hit && !fuzzy_hit {
            continue;
        }
        let rank = if prefix_hit { Rank::Prefix } else { Rank::Fuzzy };
    ...
    // 排序：前缀命中 > 模糊命中；同级内目录优先、不区分大小写名称序。
...
impl CompletionPopup {
    ...
    pub fn open(&mut self, items: Vec<Completion>, from: usize, to: usize) {
        ...
        // **同一批候选**就别把高亮打回第一条：刷新是每个按键之后都跑的，
        // 而 ↓ 之后紧接着刷一遍就会把刚移下去的高亮拽回顶部——用户看到的
        // 是"↓ 按了没反应"（`/switch ` + ↓ + 回车 提交了第一条，就是这么来的）。
        // 候选**变了**（换了筛选词/下一层）才归零，那是真正的新一轮。
        let same = self.items.len() == items.len()
            && self
                .items
                .iter()
                .zip(items.iter())
                .all(|(a, b)| a.insert == b.insert && a.name == b.name);
...
mod tests {
    ...
    /// 参数的**形状**由服务端下发（字符串，便于老前端退化），前端据此
    /// 决定给什么候选。形状认错了，Tab 就会弹出错的东西。
    #[test]
    fn arg_kinds_map_from_the_wire_strings() {
        let c = |args: &str| crate::server::wire::CommandInfo {
            name: "/x".into(),
            aliases: vec![],
            detail: String::new(),
            args: args.into(),
        ...
        assert_eq!(ArgKind::of(&c("none")), ArgKind::None);
        assert_eq!(ArgKind::of(&c("model_id")), ArgKind::ModelId);
        assert_eq!(ArgKind::of(&c("profile_name")), ArgKind::ProfileName);
        assert_eq!(ArgKind::of(&c("path")), ArgKind::Path);
        assert_eq!(ArgKind::of(&c("text")), ArgKind::Text);
        // 没见过的形状退化成"没有候选"，不猜。
        assert_eq!(ArgKind::of(&c("将来的形状")), ArgKind::None);
    }

    /// 命令表在测试里的替身：形状与服务端下发的一致（`engine` 不认识内容，
    /// 只按名字前缀过滤）。
    fn table() -> Vec<crate::server::wire::CommandInfo> {
        vec![
            crate::server::wire::CommandInfo {
                name: "/model".into(),
                aliases: vec![],
    ...
    #[test]
    fn fuzzy_subsequence_recovers_typo() {
        // 用户场景：src/ 触发后打成 "mian"，main.rs 仍要能被捞回
        //（换位 mian↔main 不是子序列，由全字符兜底捞回）。
        let d = tmpdir("fuzzy");
        fs::create_dir_all(d.join("src")).unwrap();
        fs::write(d.join("src/main.rs"), "x").unwrap();
        fs::write(d.join("src/other.txt"), "y").unwrap();

        // 精确前缀 "ma" 只中 main.rs
        let got = complete_files("src/ma", &d, &d);
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].name, "main.rs");

        // 打错成 "mian"：子序列命中 main.rs，other.txt 不中
        let got = complete_files("src/mian", &d, &d);
        let names: Vec<&str> = got.iter().map(|c| c.name.as_str()).collect();
        assert_eq!(names, vec!["main.rs"]);

        // 前缀命中排前：打 "o" 命中 other.txt（前缀）优先于任何模糊
        let got = complete_files("src/o", &d, &d);
        assert_eq!(got[0].name, "other.txt");
    ...
    fn no_hidden_file_filtering() {
        ...
        // "." 是 .env 的名字前缀（排前）；对 env.txt 是子序列命中
        // （模糊档排后）——混合过滤新契约。
        let got = complete(".", true, &d, &d, &[]);
        let names: Vec<&str> = got.iter().map(|c| c.name.as_str()).collect();
        assert_eq!(names, vec![".env", "env.txt"]);
    ...
    fn command_completion_prefix_matches() {
        let got = complete_commands("/mod", &table());
        assert_eq!(got.len(), 1);
    ...
    fn command_completion_lists_all_on_slash() {
        let got = complete_commands("/", &table());
    ...
    fn files_never_fire_on_line_start_slash_word() {
        ...
        let got = complete("/mod", true, &cwd, &cwd, &table());
        assert!(!got.is_empty());
    ...
    fn command_cascade_after_accept() {
        ...
        let got = complete_commands("/mo", &table());
        assert_eq!(got.len(), 1);
        assert!(
    ...
    fn exact_command_hit_lists_only_itself() {
        // 精确命中：只剩它自己（Enter 一步提交，不再确认补全）
        let got = complete_commands("/model", &table());
        assert_eq!(got.len(), 1);
        // 前缀命中（还在打字）：正常列出
        let got = complete_commands("/mo", &table());
        let names: Vec<String> = got.iter().map(|c| c.name.clone()).collect();
        assert_eq!(names, vec!["/model"], "前缀过滤");
    }
}

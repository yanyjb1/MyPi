...
use crate::tui::theme::{self, ColorToken};
...
/// Category → theme token. Diff colors fall back to success/error when a
/// theme omits them (all bundled themes carry them).
fn category_token(idx: usize) -> ColorToken {
    match idx {
        COMMENT => ColorToken::SyntaxComment,
        KEYWORD => ColorToken::SyntaxKeyword,
        FUNCTION => ColorToken::SyntaxFunction,
        VARIABLE => ColorToken::SyntaxVariable,
        STRING => ColorToken::SyntaxString,
        NUMBER => ColorToken::SyntaxNumber,
        TYPE => ColorToken::SyntaxType,
        OPERATOR => ColorToken::SyntaxOperator,
        PUNCTUATION => ColorToken::SyntaxPunctuation,
        INSERTED => ColorToken::ToolDiffAdded,
        DELETED => ColorToken::ToolDiffRemoved,
        _ => ColorToken::Text,
    }
}
...
fn spans_for(t: &theme::Theme, cat: usize, text: &str) -> Span<'static> {
    if cat == NOMATCH {
        return Span::raw(text.to_string());
    }
    Span::styled(text.to_string(), t.fg_style(category_token(cat)))
}
...
mod tests {
    ...
    fn rust_keywords_and_strings_get_theme_colors() {
        ...
        let all: Vec<Color> = lines
            ...
            .collect();
        assert!(
            all.contains(&t.color(ColorToken::SyntaxKeyword)),
            "keyword 色缺失: {all:?}"
        );
        assert!(
            all.contains(&t.color(ColorToken::SyntaxString)),
            "string 色缺失: {all:?}"
        );
    }
    ...
    fn comments_classify_as_comment() {
        ...
        let fgs: Vec<Color> = lines
            ...
            .flat_map(|l| l.spans.iter().map(|s| s.style.fg.unwrap_or(Color::Reset)))
            .collect();
        assert!(fgs.contains(&t.color(ColorToken::SyntaxComment)), "{fgs:?}");
    }
    ...
    fn diff_view_maps_inserted_and_deleted() {
        ...
        let fgs: Vec<Color> = lines
            ...
            .collect();
        assert!(
            fgs.contains(&t.color(ColorToken::ToolDiffAdded)),
            "inserted 色缺失: {fgs:?}"
        );
        assert!(
            fgs.contains(&t.color(ColorToken::ToolDiffRemoved)),
            "deleted 色缺失: {fgs:?}"
        );
    }

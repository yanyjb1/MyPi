...
pub struct ArtifactStore {
    inner: std::sync::Arc<std::sync::Mutex<crate::store::Store>>,
    session_id: i64,
}
...
pub fn over_threshold(result: &str) -> bool {
    result.lines().count() > SPILL_LINES || result.len() > SPILL_BYTES
}
...
mod tests {
    ...
    fn mem_store() -> ArtifactStore {
        ...
        let mut st = crate::store::Store::open(&path).unwrap();
    ...
    fn threshold_is_lines_or_bytes() {
        ...
        assert!(over_threshold(&"x".repeat(SPILL_BYTES + 1)));
    }

...
use crate::ai::types::{AssistantMessage, Context, Message, StopReason, ToolCall};
...
pub trait ToolExecutor {
    // Run one tool call, returning the text sent back to the model.
    //
...
mod tests {
    ...
    fn no_tools_reports_a_clear_error_instead_of_silently_dropping() {
        ...
        let err = exec
            .execute(&ToolCall::new("c1", "read_file", "{}"))
            .unwrap_err();
        let msg = format!("{err}");
    ...
    fn fake_executor_records_and_can_fail() {
        ...
        assert!(bad.execute(&ToolCall::new("c", "f", "{}")).is_err());
    }
